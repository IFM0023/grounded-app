 ...
 * Flow finished: localStorage onboardingCompleted must be the literal string "true" (set on final Continue); that is the only completion gate for showing onboarding.
 */
(function (global) {
  var ONBOARDING_STATE_KEY = 'grounded_onboarding_state';
  /** Only the literal string "true" means the user finished onboarding (final Continue). */
  var ONBOARDING_COMPLETED_KEY = 'onboardingCompleted';
  var STATE_VERSION = 3;

  function readOnboardingCompletedExplicit() {
    try {
  ...
  function normalizeState(o) {
    ...
    return {
      ...
      userName: uname,
      onboardingCompleted: readOnboardingCompletedExplicit()
    };
  }
   ...
   * sessionStorage marks a replay session so other logic can treat this visit specially.
  ...
  /**
   * Legacy helper kept for API compatibility. Onboarding is only marked complete when
   * the user taps the final Continue (see onboarding.js); prior site visits must not skip it.
   */
  ...
  function shouldShowOnboardingUI() {
    return !readOnboardingCompletedExplicit();
  }
  ...
  function bootstrapTheme(setThemeFn) {
    ...
    if (readOnboardingCompletedExplicit() && s.selectedTheme) {
  ...
  global.GroundedThemes = {
    ...
    ONBOARDING_COMPLETED_KEY: ONBOARDING_COMPLETED_KEY,
    readOnboardingCompletedExplicit: readOnboardingCompletedExplicit,
